...
import { Activity, Download, RefreshCw, Copy } from "lucide-react";
...
type ReviewSource = "g2" | "capterra" | "trustradius";
type SourcesState = Record<ReviewSource, boolean>;

const SOURCE_OPTIONS: Array<{ key: ReviewSource; label: string; color: string }> = [
...
const ReviewScraperUI = () => {
  ...
  // IMPORTANT: give generics so TS doesn't infer never[]
  ...
  const handleSourceToggle = (source: ReviewSource) => {
    setSources((prev) => ({
      ...prev,
      [source]: !prev[source],
    }));
  ...
  const handleInitializeScrape = () => {
    // your original condition was odd; keeping your behavior but making it typed-safe
    if (!company && !startDate && !endDate) {
      const selectedSources = (Object.keys(sources) as ReviewSource[]).filter((k) => sources[k]);
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: "1rem", alignItems: "center", flex: 1, marginLeft: "3rem" }}>
              <input
                ...
                placeholder="Type command... eg. Scrape Notion from Feb 2024 to May 2024"
                style={{
                  flex: 1,
                  background: "#1f1f1f",
                  border: "1px solid #3a3a3a",
          ...
          {SOURCE_OPTIONS.map((source) => (
            <button
              ...
              style={{
                ...
                height: "110px",
                borderRadius: "50%",
                background: sources[source.key]
                  ? `linear-gradient(135deg, ${source.color}dd, ${source.color})`
                  : "#1f1f1f",
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
              <svg width="100%" height="180" viewBox="0 0 700 180" preserveAspectRatio="none">
                <polyline
                  points={
                    pulseData.length > 0
                      ? pulseData.map((val, i) => `${i * 17.5},${180 - val * 1.5}`).join(" ")
                      : "0,90 700,90"
                  }
                  fill="none"
                  stroke="#00D9FF"
                ...
                {pulseData.length > 15 && (
                  <circle
                    cx={pulseData.length * 17.5 - 350}
                    cy={180 - pulseData[Math.floor(pulseData.length * 0.6)] * 1.5}
                    r="6"
                    fill="#00D9FF"
                    opacity="0.8"
                  />
        ...
        {showResults && (
          <div style={{ marginBottom: "2rem" }}>
            <div style={{ background: "#1f1f1f", padding: "2rem", borderRadius: "12px", border: "1px solid #3a3a3a", marginBottom: "2rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: "2rem" }}>
              <div style={{ background: "#1f1f1f", padding: "1.5rem", borderRadius: "12px", border: "1px solid #3a3a3a" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", alignContent: "start" }}>
                {sampleReviews.map((review, i) => (
                  <div
                    key={i}
                    style={{ background: "#1f1f1f", padding: "1rem", borderRadius: "8px", border: "1px solid #3a3a3a", minHeight: "100px" }}
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem", gap: "0.5rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.7rem", color: "#888", flex: 1, minWidth: 0 }}>
                        <div style={{ width: "20px", height: "20px", borderRadius: "50%", background: i % 3 === 0 ? "#FF3366" : i % 3 === 1 ? "#00D9FF" : "#FF9500", flexShrink: 0 }} />
                        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{review.author}</span>
                      </div>
                      {review.date && (
                        <div style={{ fontSize: "0.65rem", color: "#666", whiteSpace: "nowrap", flexShrink: 0 }}>{review.date}</div>
                      )}
                    </div>

                    <p style={{ color: "#ccc", fontSize: "0.75rem", margin: 0, lineHeight: 1.5, marginBottom: "0.75rem" }}>{review.title}</p>
